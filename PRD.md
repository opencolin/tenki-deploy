...
>
> **Tenki as the default deploy target** (2026-08-01): agents inside demo sandboxes publish to Tenki rather than GitHub Pages / Vercel / a tunnel. Implemented with the **Tenki CLI, not the MCP server**: `@tenkicloud/mcp` isn't on npm (404) and has no `prepare` script, so it needs clone + `npm install` + `tsc` inside a 1-vCPU VM and three different per-agent config formats; the CLI installs in ~2s via `install.sh`, authenticates from `TENKI_AUTH_TOKEN` with no config file, and works identically across all three agents (they all run shell). The broker preinstalls the CLI at sandbox creation, exports `TENKI_AUTH_TOKEN` + `TENKI_SESSION_ID` into the launcher, and writes `AGENTS.md`/`CLAUDE.md` with the deploy recipe (serve on a port → `tenki sandbox expose`). Exposable ports: 3000/5000/8000/8081/9000 (8080 = ttyd, 7681 = base-image ttyd, both rejected). Verified e2e: an agent built and published a snake game to a live public URL in 41s. MCP remains the better fit for real users on their own machines, once published to npm.
>
